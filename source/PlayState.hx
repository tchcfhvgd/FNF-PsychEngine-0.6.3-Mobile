...
class PlayState extends MusicBeatState
{
  ...
  scripts.executeAllFunc("ghostTap", [key]);
	...
	function initScripts()
	{
		...
		if (FileSystem.exists("mods/scripts/global"))
		{
			for (_ in ScriptUtil.findScriptsInDir("mods/scripts/global"))
				files.push(_);
		}
	...
	{
		if (!FileSystem.exists("mods/scripts/events"))
			return;

		var jsonFiles:Array<String> = CoolUtil.findFilesInPath("mods/scripts/events", ["json"], true, false);

		var hxFiles:Map<String, String> = [];

		if (FileSystem.exists('mods/scripts/events/${Paths.formatToSongPath(SONG.song)}'))
		{
			for (file in CoolUtil.findFilesInPath('mods/scripts/events/${Paths.formatToSongPath(SONG.song)}', ["json"], true, true))
				jsonFiles.push(file);
		}
