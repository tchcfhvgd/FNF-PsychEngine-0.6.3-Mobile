...
function onCreate()
	...
	makeLuaSprite('bgThing', 'songBG8', -550, 410)
    ...
    if count < 15 then
        makeLuaText('songText', "".. useName.. " - ".. toughness, 400, getProperty('bgThing.x') + 180, 510) -- 20 letters is the width limit
        ...
        makeIt = true
        makeLuaText('songText', "".. useName, 400, getProperty('bgThing.x') + 180, 480) -- Too big song name
        setObjectCamera("songText", 'other');
        setTextColor('songText', '0x'..songTxtColor)
        setTextSize('songText', 40);
        addLuaText("songText");
    ...
    if makeIt == true then -- too big of a song name
        makeLuaText('optionalText', " / ".. toughnessSmal, 400, getProperty('bgThing.x') + 180, 510)
    ...
    if showCredit == true then
        makeLuaText('whoDuntIt', "Made by - ".. name, 400, getProperty('bgThing.x') + 180, 600)
...
function onCreatePost()
    if getProperty('inCutscene') == false then
        doTweenX('bgThingMoveIn', 'bgThing', -50, 1, 'linear')
        doTweenX('bgThingText', 'songText', 50, 1, 'linear') 
        doTweenX('bgThingTextBleb', 'beforeSongText', 20, 1, 'linear')

        if madeCred == true then
            doTweenX('bgThingCredit', 'whoDuntIt', 70, 1, 'linear')
        end
        if makeIt == true then
            doTweenX('bgThingOptTextIn', 'optionalText', 350, 1, 'linear')
        end
...
function onStartCountdown(count)
    if countIt == true and startedIt == false then
        startedIt = true
        doTweenX('bgThingMoveInL8', 'bgThing', -50, 0.5, 'linear')
        doTweenX('bgThingTextL8', 'songText', 50, 0.5, 'linear') 
        doTweenX('bgThingTextBlebL8', 'beforeSongText', 20, 0.5, 'linear')
        
        if madeCred == true then
            doTweenX('bgThingCreditL8', 'whoDuntIt', 70, 0.5, 'linear')
        end
        if makeIt == true then
            doTweenX('bgThingOptTextInL8', 'optionalText', 350, 0.5, 'linear')
        end
...
function onTimerCompleted(tag, loops, loopsLeft)
    if tag == 'moveOut' then
        doTweenX('bgThingLeave', 'bgThing', -10000, 3.5, 'linear')
        doTweenX('bgThingLeaveText', 'songText', -10000, 3.5, 'linear')
        doTweenX('bgThingLeavePreText', 'beforeSongText', -10000, 3.5, 'linear')

        if madeCred == true then
            doTweenX('bgThingLeaveCredit', 'whoDuntIt', -10000, 3.5, 'linear')
        end
        if makeIt == true then
            doTweenX('bgThingOptTextBye', 'optionalText', -10000, 3.5, 'linear') -- long ass name
        end
    end
end
